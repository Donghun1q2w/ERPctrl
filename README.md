# erpctrl — Claude Code / Codex 플러그인

SHIIS3 ERP UI 자동화 CLI(`erpctrl.exe`)를 **자연어로 구동**하는 Claude Code / Codex 겸용 플러그인.
...
| 스킬 본문 | `skills/**/SKILL.md` 공용 | `skills/**/SKILL.md` 공용 |
| 설치 명령 | `/plugin ...` | `codex plugin ...` |
| CLI 바이너리 | 번들하지 않음, 실행 시 공유서버에서 부트스트랩 | 동일 |

즉, 실제 자동화 절차는 두 환경이 같은 `skills/`를 읽고, 각 도구별 매니페스트만 분리한다.
...
이 저장소는 Claude Code용 자기참조 마켓플레이스를 포함한다.
...
Codex는 `.codex-plugin/plugin.json`을 읽는다. 표준 로컬 마켓플레이스 구조는 다음과 같다.
...
`<marketplace-root>/.agents/plugins/marketplace.json` 예시:

```json
{
  "name": "erpctrl-local",
  "interface": {
    "displayName": "ERPCTRL Local"
  },
  "plugins": [
    {
      "name": "erpctrl",
...
codex plugin add erpctrl@erpctrl-local
```

이미 기본 개인 마켓플레이스(`~/.agents/plugins/marketplace.json`)에서 `plugins/erpctrl`을 가리키고 있다면
마켓플레이스 추가 없이 `codex plugin add erpctrl@<marketplace-name>`만 실행하면 된다.
